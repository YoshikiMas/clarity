...
class SceneRenderer:
    ...
    def generate_hoa_signals(self, scene):
        ...
        norm = np.max(np.abs(hoa_target))
        logger.info(f"max_hoa_target: {norm}")
        norm = np.max(np.abs(hoa_target_anechoic))
        logger.info(f"max_hoa_target_anechoic: {norm}")
    ...
    def generate_binaural_signals(
        ...
        norm = np.max([np.max(np.abs(sig)) for sig in targets[1:]])
        logger.info(f"max_bin_target: {norm}")
        norm = np.max([np.max(np.abs(sig)) for sig in target_anechoics[1:]])
        logger.info(f"max_bin_target_anechoic: {norm}")

        maxvals = []
        maxvals += [np.max(np.abs(sig)) for sig in mix[1:]]  # ch0 is not used
        maxvals += [np.max(np.abs(sig)) for sig in targets[1:]]
        maxvals += [np.max(np.abs(sig)) for sig in target_anechoics[1:]]
        maxvals += [np.max(np.abs(sig)) for sig in interferers[1:]]

        for interferer_k in all_interferers:
            maxvals += [np.max(np.abs(sig)) for sig in interferer_k[1:]]

        # norm = max(maxvals + self.channel_norms[1:])
        norm = 2.0
        ...
        # Save the anechoic signal at all microphones.
        # norm = np.max(np.abs(target_anechoics[ref_chan]))
        # self.save_signal_16bit(
        #     f"{file_stem}_target_anechoic_CH{ref_chan}.wav", target_anechoics[ref_chan], norm
        # )
        for channel, sig in enumerate(target_anechoics):
            self.save_signal_16bit(
                f"{file_stem}_target_anechoic_CH{channel}.wav", sig, norm
            )

        # Save each interferer
        for k, interferer_k in enumerate(all_interferers):
            for channel, sig in enumerate(interferer_k):
                self.save_signal_16bit(
